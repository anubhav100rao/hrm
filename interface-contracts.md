...
Example payloads that share IDs (for example `anom_01JZ4Q0XB...` / `dec_01JZ4Q12...`) belong to a single end-to-end storyline: a payroll anomaly for `E0412` whose RL-ranked action is `escalate-to-HR`, which a reviewer then modifies. Examples that illustrate a *different* branch (such as a correction-tier approval on `auto-correct`) use their own distinct IDs so no single decision ever shows two conflicting outcomes.
...
IDs use ULIDs so they sort by creation time and stay readable in traces:
...
## 2. Canonical HR Data Models

These are canonicalized rows read from HRIS, payroll, attendance, leave, and learning systems. Production detectors read from operational stores, warehouse tables, or a feature store, but normalize records into these shapes before scoring.
...
```json
{
  ...
  "audit_label_internal": {
    "label": "payroll_outlier",
    "source": "payroll_audit",
    "reason": "unexpected deduction spike of AED 800",
    "available_to_runtime_agents": false
  }
}
```
...
`Intent` is the in-state representation of a classified request. For reactive signals it is produced by mapping the Supervisor's `SupervisorTriageOutput` (see [llm-instructions.md](llm-instructions.md), section 1) field-for-field; `missing_fields` from triage drives clarification while `user_text` is copied from the originating signal payload. Scheduled and system signals construct `Intent` deterministically without an LLM call.
...
Policy chunks are stored in the managed policy vector index. The LLM sees only retrieved chunks, not the whole policy book.
...
The bandit ranks the same action candidates produced by the deterministic prior. It persists both the chosen action and all scores for diagnostics.
...
[
  {
    ...
    "source": "hitl",
    "value": 0.1,
    "reason": "HR reviewer kept the escalate-to-HR action but modified its params (small edit distance), so partial reward",
    "observed_at": "2026-06-18T10:22:51Z",
    "metadata": {"reviewer_id": "hr_ops_17", "decision": "modify"}
  },
  {
...
Example compliance approval requirement (separate incident: here RL surfaced `auto-correct`, which trips the correction-tier rule):

```json
{
  "proposed_action": {
    "decision_id": "dec_01JZ4Q0PB2CDXNQZNE9EFYE24C",
    "anomaly_id": "anom_01JZ4Q0PB1AHZCSJEB69JWJ8NW",
    "action_type": "auto-correct",
    "params": {"employee_id": "E0731", "period": "2026-05", "delta": 800.0, "currency": "AED"},
    "risk_tier": "medium",
    "source": "rl",
  ...
  "safe_fallback": {
    "action_type": "flag-for-audit",
    "params": {"employee_id": "E0731", "period": "2026-05"},
    "risk_tier": "low",
...
The approval packet is the exact object persisted to the approval store and displayed in the reviewer console.
...
```json
{
  ...
  "reward_total": 0.4,
  "outcome": "resolved",
  "resolution_summary": "HR escalation created ticket TCK-12991. RL ranked escalate-to-HR first; the reviewer kept that action but modified its params to require deduction-code owner verification before any correction.",
...
Example managed vector memory document:

```json
{
  "id": "inc_01JZ4Q8YBFGW9DCK7DPE2FS31N",
  "document": "payroll anomaly | AE | B3 | net pay dropped 500-1000 AED | no matching approved deduction | action escalate-to-HR | reviewer modified escalation params | outcome resolved",
  "metadata": {
    "anomaly_type": "payroll",
    "employee_country": "AE",
    "employee_band": "B3",
